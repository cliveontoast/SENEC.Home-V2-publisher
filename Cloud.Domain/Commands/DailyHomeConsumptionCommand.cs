    ...
    public class DailyHomeConsumptionCommandHandler : IRequestHandler<DailyHomeConsumptionCommand, HomeConsumptionDaily>
    {
        ...
        public async Task<HomeConsumptionDaily> Handle(DailyHomeConsumptionCommand request, CancellationToken cancellationToken)
        {
            ...
            var results = new HomeConsumptionDaily(
                date: request.Date,
                consumption: GetData(request.Date, dayData, a => a.ToHome ?? a.ConsumptionWattHours),
                grid: GetData(request.Date, dayData, a => a.GridImportWattHours),
                solarConsumption: GetData(request.Date, dayData, a => a.SolarConsumption ?? Math.Min(a.ConsumptionWattHours - a.GridImportWattHours, a.SolarPowerGenerationWattHours)),
                ...
                solarExported: GetData(request.Date, dayData, a => a.SolarToCommunity ?? Math.Max(0, a.SolarPowerGenerationWattHours 
